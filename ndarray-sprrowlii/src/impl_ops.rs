        ...
        if self.ndim() == rhs.ndim() && self.shape() == rhs.shape() {
            if self.is_standard_layout() {
                if rhs.is_standard_layout() {
                    let shape = <D as DimMax<E>>::Output::from_dimension(&self.raw_dim()).unwrap();
                    let output = Array::uninit(shape);
                    unsafe {
                        if std::any::TypeId::of::<A>() == std::any::TypeId::of::<f64>() {
                            ndarray_simd::mul_512_f64(self.ptr.as_ptr() as *const f64, rhs.ptr.as_ptr() as *const f64, output.ptr.as_ptr() as *mut f64, self.raw_dim().size());
                        }
                        if std::any::TypeId::of::<A>() == std::any::TypeId::of::<i32>() {
                            ndarray_simd::mul_512_i32(self.ptr.as_ptr() as *const i32, rhs.ptr.as_ptr() as *const i32, output.ptr.as_ptr() as *mut i32, self.raw_dim().size());
                        }
...
mod arithmetic_ops {
    ...
    impl_mul_op!(Mul, *, mul, *=, "multiplication");
